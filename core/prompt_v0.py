...
BELANGRIJK:
Na het kiezen van het gerecht moet je deze bereidingstijd ook expliciet vastleggen in het JSON veld:
...
Gebruik de gekozen time-mapping om realistische minuten te bepalen.

Voorbeelden:
- kort → { "min": 15, "max": 20 }
- normaal → { "min": 30, "max": 45 }
- lang → { "min": 50, "max": 90 }

Dit veld is verplicht en mag nooit ontbreken.
...
────────────────────
REGELS VOOR KOOKTIJD
────────────────────

- cook_time.min en cook_time.max zijn minuten (getallen)
- Als het gerecht ongeveer even lang duurt:
  zet min en max gelijk (bijv: 30 en 30)
- Gebruik altijd realistische tijden
...
Gebruik altijd gestructureerde objecten met:

- amount → alleen hoeveelheid + eenheid  
  (bijv. "150 g", "1 teen", "2 tl", "1 stuk", "2 cm")

- item → alleen de naam van het ingrediënt  
  (bijv. "Aubergine", "Bulgur", "Knoflook")

- note → alle extra info zoals:
  snijwijze, bereidingsvorm, uit blik, droog, optioneel, etc.

Regels:

• amount bevat NOOIT haakjes, komma’s of beschrijvingen  
• Zet nooit extra uitleg in item  
...
{ "amount": "250 g", "item": "Aubergine", "note": "in blokjes" }
...
- calories_kcal = totale calorieën gerecht
- protein_g, fat_g, carbs_g = grammen
- macro_ratio berekenen met:
  - eiwit = 4 kcal per gram
  - koolhydraten = 4 kcal per gram
  - vet = 9 kcal per gram
- macro_ratio percentages samen ≈ 100%

────────────────────
REGELS VOOR BEREIDING
────────────────────

- steps is een geordende lijst
- elke stap is één duidelijke handeling
- geen lege strings
...
❗ Geef NOOIT uitleg of tekst buiten de JSON  
❗ Volg exact deze structuur  
❗ Gebruik altijd cook_time  
