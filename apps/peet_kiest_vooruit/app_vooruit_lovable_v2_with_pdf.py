...
# ---------- PDF ----------
def render_pdf(days_data):
    output_dir = "output"
    os.makedirs(output_dir, exist_ok=True)
    path = os.path.join(
        output_dir,
        f"Peet_Kiest_Vooruit_{date.today().strftime('%Y%m%d')}.pdf"
    )
    ...
    for i, day in enumerate(days_data):
        ...
        c.drawString(40, y, f"Dag {day['day']} – {day['screen8']['dish_name']}")
        y -= 30
        ...
        c.setFont("Helvetica", 10)
        for step in day["recipe"]["steps"]:
            lines = simpleSplit(step["text"], "Helvetica", 10, width - 90)
            for line in lines:
...
pdf_path = render_pdf(days_data)

with open(pdf_path, "rb") as f:
    st.download_button(
        "Bekijk recept & boodschappen (PDF)",
        data=f,
